...
<head>
    ...
    <style>
        *{font-family:'Plus Jakarta Sans',sans-serif;box-sizing:border-box;}
        ...
        .logo-ring{width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,#7f1d1d,#dc2626);display:flex;align-items:center;justify-content:center;margin:0 auto 1rem;box-shadow:0 8px 32px rgba(185,28,28,0.6),0 0 0 5px rgba(185,28,28,0.15);overflow:hidden;}
...
<div class="w-full max-w-md relative z-10 my-6">
    <div class="text-center mb-6 fu">
        <div class="logo-ring">
        <img src="{{ asset('images/ais-logo.png') }}" alt="AIS" style="width:100%;height:100%;object-fit:cover;">
        </div>
        <h1 style="font-family:'Playfair Display',serif;font-size:1.5rem;font-weight:800;color:#fff;">Ambalayat Integrated School</h1>
        <p style="color:rgba(252,165,165,0.75);font-size:0.78rem;font-weight:500;margin-top:0.25rem;letter-spacing:0.07em;text-transform:uppercase;">Document Tracking System</p>
    </div>

    <div class="card fu2">
        <div class="accent-bar"></div>
        <div style="padding:2rem;">
            <h2 style="font-size:1.25rem;font-weight:800;color:#0f172a;margin-bottom:0.25rem;">Create Account</h2>
            <p style="color:#64748b;font-size:0.875rem;margin-bottom:1.5rem;">Register your staff account to get started</p>

            @if($errors->any())
            <div style="background:#fef2f2;border-left:4px solid #dc2626;border-radius:0.625rem;padding:0.875rem 1rem;margin-bottom:1.25rem;">
                <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:0.25rem;">
                    @foreach($errors->all() as $error)
                    <li style="color:#991b1b;font-size:0.8125rem;font-weight:500;display:flex;align-items:center;gap:0.375rem;">
                        <span style="width:5px;height:5px;background:#dc2626;border-radius:50%;flex-shrink:0;display:inline-block;"></span>{{ $error }}
                    </li>
                    @endforeach
                </ul>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}" style="display:flex;flex-direction:column;gap:1rem;">
                ...
                <!-- Name -->
                <div>
                    <label style="display:block;font-size:0.8125rem;font-weight:600;color:#374151;margin-bottom:0.4rem;">Full Name</label>
                    <div class="input-group">
                        <div class="ig-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div>
                ...
                <!-- Position -->
                <div>
                    <label style="display:block;font-size:0.8125rem;font-weight:600;color:#374151;margin-bottom:0.4rem;">Position / Role</label>
                    <div class="input-group">
                        <div class="ig-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg></div>
                        <input type="text" name="position" value="{{ old('position') }}" required placeholder="e.g. Teacher, Clerk, Staff">
                    </div>
                </div>
                <!-- Password -->
                <div>
                    ...
                    <div class="input-group">
                        ...
                        <input type="password" name="password" id="pw1" required placeholder="Minimum 8 characters">
                        <button type="button" class="ig-action" onclick="togglePw('pw1','e1a','e1b')">
                            <svg id="e1a" style="width:1.125rem;height:1.125rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            <svg id="e1b" style="width:1.125rem;height:1.125rem;display:none;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                        </button>
                    </div>
                </div>
                <!-- Confirm Password -->
                <div>
                    <label style="display:block;font-size:0.8125rem;font-weight:600;color:#374151;margin-bottom:0.4rem;">Confirm Password</label>
                    <div class="input-group">
                        <div class="ig-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg></div>
                        <input type="password" name="password_confirmation" id="pw2" required placeholder="Repeat your password">
                        <button type="button" class="ig-action" onclick="togglePw('pw2','e2a','e2b')">
                            <svg id="e2a" style="width:1.125rem;height:1.125rem;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            <svg id="e2b" style="width:1.125rem;height:1.125rem;display:none;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                        </button>
                    </div>
                </div>
                <button type="submit" class="btn-main" style="margin-top:0.25rem;">Create My Account</button>
                <p style="text-align:center;font-size:0.875rem;color:#64748b;">Already have an account? <a href="{{ route('login') }}" style="color:#991b1b;font-weight:700;text-decoration:none;">Sign in here</a></p>
            </form>
        </div>
    </div>
    <p style="text-align:center;color:rgba(252,165,165,0.35);font-size:0.7rem;margin-top:1.25rem;">© {{ date('Y') }} Ambalayat Integrated School · All rights reserved</p>
</div>
<script>
